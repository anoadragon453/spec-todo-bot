...
The docker image will run spec-todo-bot with a SQLite database and
...
* Choose whether you want to use SQLite or Postgres as your database backend.
  ...
  ```
  database: "postgres://username:password@postgres/spec-todo-bot?sslmode=disable"
  ```
...
```
docker-compose up spec-todo-bot
```
...
```
docker-compose pull spec-todo-bot
```
...
[spec-todo-bot.service](spec-todo-bot.service). The service uses
`docker-compose` to start and stop the bot.

Copy the file to `/etc/systemd/system/spec-todo-bot.service` and edit to
match your setup. You can then start the bot with:

```
systemctl start spec-todo-bot
```

and stop it with:

```
systemctl stop spec-todo-bot
```

To run the bot on system startup:

```
systemctl enable spec-todo-bot
```
...
```
docker build -t somebody/spec-todo-bot:latest -f docker/Dockerfile .
```
